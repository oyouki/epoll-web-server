...
int server_parse_msg(char *buf,size_t *buflen,FdContext *fdc);    
...
void work_ser2cli_reg_fail(char *buf,size_t *len,FdContext *fdc){
    fdc->client.task_buf_pos=0;     //任务缓冲清空
    fdc->client.status=S_CLI_CONN;
    set_cmd(buf,CMD_SER2CLI_REG_FAIL);
    build_lpmsg_inplace(buf,CMD_CODELEN,PREFIX_LENGTH);
    *len=CMD_CODELEN+PREFIX_LENGTH;
}

int server_parse_msg(char *buf,size_t *buflen,FdContext* fdc){   //载荷:buf  buflen:载荷长度
    int cmd=get_cmd(buf);
    switch(cmd){
        case CMD_CLI2SER_REG:
            if(fdc->client.status!=S_CLI_CONN)  return -1;
            work_ser2db_reg(buf,buflen,fdc);
            return 1;
            break;
        case CMD_CLI2SER_LOGIN:         //client身份认证
            //work_ser2db_login(buf,buflen,fdc);
            return 1;
            break;
        case CMD_CLI2SER_GETINFO:         //client获取信息
            break;
        case CMD_CLI2SER_DOWNLOAD:         //client下载
            //此处需要读取文件编号,以及文件路径和编号的映射转换(需要数据库)
        case CMD_DB2SER_REG_FAIL:
            work_ser2cli_reg_fail(buf,buflen,fdc);
            return 0;
        default:
            ;
    }
